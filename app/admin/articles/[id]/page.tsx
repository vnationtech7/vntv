...
export default function ArticleEditorPage() {
  ...
  const dataLoadedRef = useRef(false); // Track if dropdown data has been loaded
  ...
  useEffect(() => {
    ...
    dataLoadedRef.current = true; // Mark as loading to prevent duplicates
    ...
    if (!isNew && articleId) {
      const loadArticle = async () => {
        try {
          ...
          const { data: articleTags } = await getArticleTags(articleId);
          ...
          if (article) {
            setFormData({
              ...
              sponsor_label: article.sponsor_label || "",
  ...
  }, [isNew, articleId]); // Only depend on isNew and articleId
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = isNew
        ...
        : await updateArticle(articleId!, submissionData);
      ...
      <PageHeader
        ...
        description={
          isNew
            ...
            : "Edit article details and content"
        }
        action={
