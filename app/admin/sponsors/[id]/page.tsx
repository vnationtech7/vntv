...
export default function EditSponsorPage({
  ...
  return (
    <RequireRole allowedRoles={["super_admin", "advertising_manager"]}>
      <EditSponsorPageContent id={id} />
    </RequireRole>
  );
}

function EditSponsorPageContent({ id }: { id: string }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      const { data } = await getSponsorship(id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data, error } = await updateSponsorship(id, {
      name: formData.name,
    ...
    return (
      <AdminLayout>
        <div className="p-6 text-center">
          <p className="text-text-tertiary mb-4">Sponsor not found</p>
  ...
  return (
    <AdminLayout>
      <PageHeader
        title={`Edit Sponsor: ${sponsor.name}`}
