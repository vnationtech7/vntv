...
import { Mail, Lock, User, Loader } from "@/components/icons";
...
export function SignupModal({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (result?.error) {
      ...
      // Success - call onSuccess callback or refresh
      if (onSuccess) {
        onSuccess();
      } else {
        onClose(false);
  ...
  const content = (
    <div className="space-y-4">
        ...
        <Checkbox
          ...
          label={
            <span className="text-[12px]">
              I agree to the{" "}
              <a href="/terms" className="text-[--color-vntv-red] hover:underline">
                Terms of Service
              </a>{" "}
              and{" "}
              <a
      ...
      {/* Links */}
      {onSwitchToLogin && (
        <div className="text-center text-[13px] text-[--color-foreground-muted]">
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent size="sm">
        <DialogHeader>
          <DialogTitle>
            Join <span className="text-[--color-vntv-red]">VN</span>TV
          </DialogTitle>
